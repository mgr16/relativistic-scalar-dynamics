...
def test_postprocess_fft_outputs(tmp_path, monkeypatch):
    from rsd.cli import postprocess_main

    run_dir = tmp_path / "run"
    _write_series(run_dir)
    monkeypatch.setattr(sys, "argv", ["rsd-postprocess", "--run", str(run_dir), "--qnm", "--method", "fft"])
...
def test_postprocess_prony_outputs(tmp_path, monkeypatch):
    ...
    _write_series(run_dir)
    monkeypatch.setattr(
        sys, "argv", ["rsd-postprocess", "--run", str(run_dir), "--qnm", "--method", "prony", "--modes", "2"]
    )
    assert postprocess_main() == 0
